...
export class RDSData {
  ...
  public async queryBatch(sql: string, params: Array<RDSDataParameters>, transactionId?: string): Promise<RDSDataService.BatchExecuteStatementResponse> {
    const parameters = params.map(row => RDSData.formatParameters(row));
